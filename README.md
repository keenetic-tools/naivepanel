...
```bash
curl -fsSL https://raw.githubusercontent.com/keenetic-tools/naivepanel/v0.8.1/install.sh | sh -s -- --with-auth
```
...
| `--ref TAG` | устанавливает конкретный тег (по умолчанию `v0.8.1`) |
...
```bash
curl -fsSL https://raw.githubusercontent.com/keenetic-tools/naivepanel/v0.8.1/install.sh \
